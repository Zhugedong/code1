...
int main(int argc, char **argv)
{
	...
	if(!backbone.empty()){
		cv::Mat frame;
		std::cout<<"Capture video stream from capture card: " << backbone << std::endl;
		cv::VideoCapture capture("/home/lizhixue/project/video/lxg2333642.mp4");  
		capture.set(cv::CAP_PROP_FRAME_WIDTH, 1920);
        	capture.set(cv::CAP_PROP_FRAME_HEIGHT, 1080);
        	std::cout << "mark" << std::endl;
        	if (!capture.isOpened()) {
        	    printf("could not read from this video capture device...\n");
       		    return -1;
        	} 
	}
